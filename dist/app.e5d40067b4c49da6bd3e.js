...
var omnibox = __webpack_require__(1);
var UrlPad;
(function (UrlPad) {
    var map = function (o) {
        return {
            ...
            host: o.host,
            port: o.port,
            pathnames: o.pathname.split('/').filter(function (char) { return char; }),
            hash: o.hash,
            query: o.query
        };
    };
    var parse_querystrings = function (qs) {
        return Object.keys(qs)
            .map(function (key, index) { return "<input type=\"text\" value='" + key + "'><input type=\"text\" value='" + qs[key] + "'><br/>"; })
            .join('');
    };
    var parse_pathnames = function (names) {
        return names.map(function (v, i) { return "<label>Pathname#" + i + "</label><input type=\"text\" value='" + v + "'><br/>"; }).join('');
    };
    function parse(url) {
        var o = map(omnibox.parse(url));
        return Object.keys(o)
            .filter(function (key, index) { return o[key]; })
